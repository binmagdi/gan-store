...
const ImageUpload: FC<ImageUploadProps> = ({
  ...
  if (type === "profile") {
    return (
      <div className="relative rounded-full w-52 h-52 bg-gray-200 border-2 border-white shadow-md  ">
        {(value ?? []).length > 0 && (
          <Image
            src={(value ?? [])[0]}
            alt=""
        ...
        <CldUploadWidget onSuccess={onUpload} uploadPreset="ganstore">
          {({ open }) => {
            const onClick = () => {
              open();
            };
            ...
            return (
              ...
              >
                ...
                <span className="capitalize">
                  {(value?.length ?? 0) > 0 ? "Change cover" : "Upload a cover"}
                </span>
              </button>
            );
          }}
          ...
          {({ open }) => {
            ...
            return (
              <>
                <button
                  type="button"
                  className="flex items-center font-medium text-[17px] py-3 px-6 text-white bg-gradient-to-t from-blue-primary to-blue-300 border-none shadow-md rounded-full hover:shadow-sm active:shadow-sm"
                  disabled={disabled}
                  onClick={onClick}
                >
                  <svg
